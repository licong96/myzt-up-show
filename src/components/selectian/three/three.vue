...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      submit () {
        if (!this.preference.companyName || !this.preference.companyPost || !this.preference.territory.length || !this.preference.stage.length) {
          this.setMuDialog(true)
          this.setMuDialogText('请把资料填写完整再提交')
          this.setMuDialogUrl('')
        } else {
          let self = this
          this.axios.post('/api/user/entrepreneur', this.preference)
          .then(function (response) {
