  ...
  export default {
    ...
    methods: {
        ...
        this.axios.get('/api/user/geetest_show_verify')
          .then(function (response) {
            window.initGeetest({
              ...
              self.captchaObj.appendTo(self.$refs.captcha)
              self.captchaObj.onReady(function() {
                self.captchaWait = false
              })
          ...
          .catch(function () {
            self.dialogText = '网络错误，请检查你的网络状态，或者刷新试试'
            self.dialog = true
          })
      },
      postPhone() {       // 发送手机号到后端，已接受验证码
        ...
        console.log(data)
        // let myFrom = this.$refs.myform.serialize()
        this.axios.post('/api/user/geetest_ajax_check', data)
        .then(function (response) {
